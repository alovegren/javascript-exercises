...
// We didn't define the function with any arguments. One fix would be to use a rest parameter to create a handle on whatever arguments passed into the function.

// Another thing to realize is that while the `arguments` object is available whether we explicitly include arguments in the function signature or not, the `arguments` object is not an Array and does not respond to `Array.prototype.forEach`.